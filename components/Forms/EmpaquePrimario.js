...
export default function FormularioAfiliado({ color, readonly = false, idInformacionF: propIdInformacionF, estado: propEstado }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    if (!idInformacionF) {
      alert("No se encontró idInformacionF para guardar Empaque Primario.");
      setIsLoading(false);
      return;
    }
    if (productos.length === 0) {
      try {
        ...
        const url = `${API_BASE_URL}/informacion-f/crearEmpaquePri?importId=${encodeURIComponent(importId)}&batchIndex=0&batchCount=1&mode=replace&idInformacionF=${encodeURIComponent(idInformacionF)}`;
        const response = await fetch(url, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
        ...
        alert("Se eliminaron todos los registros de Empaque Primario.");
        await fetchToneladasAcumuladas();
      } catch (error) {
        console.error("Error al guardar Empaque Primario vacío:", error);
  ...
  const concurrency = 2; // menor paralelismo para reducir presión
      ...
      const uploadChunk = async (index) => {
        ...
        const url = `${API_BASE_URL}/informacion-f/crearEmpaquePri?importId=${encodeURIComponent(importId)}&batchIndex=${index}&batchCount=${totalChunks}&mode=${isFirst ? 'replace' : 'append'}&idInformacionF=${encodeURIComponent(idInformacionF)}`;
        const response = await fetch(url, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
