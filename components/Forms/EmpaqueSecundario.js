...
export default function FormularioAfiliado({ color, readonly = false, idInformacionF: propIdInformacionF, estado: propEstado }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    if (!idInformacionF) {
      alert("No se encontró idInformacionF para guardar Empaque Secundario.");
      setIsLoading(false);
      return;
    }
    if (productos.length === 0) {
      try {
        ...
        const url = `${API_BASE_URL}/informacion-f/crearEmpaqueSec?importId=${encodeURIComponent(importId)}&batchIndex=0&batchCount=1&mode=replace&idInformacionF=${encodeURIComponent(idInformacionF)}`;
  ...
  if (/[,.]/.test(producto.unidades)) {
    ...
    try {
      ...
      const resolveServerPacketLimit = () => {
        ...
        console.warn('[EmpaqueSecundario] Límite por defecto 536870912 bytes (512MB). Si cambió max_allowed_packet ejecute localStorage.setItem("serverMaxPacketBytes", "NUEVO_VALOR") para usar valor personalizado.');
        return 536870912;
      };
      ...
      const uploadChunk = async (index) => {
        ...
        const url = `${API_BASE_URL}/informacion-f/crearEmpaqueSec?importId=${encodeURIComponent(importId)}&batchIndex=${index}&batchCount=${totalChunks}&mode=${isFirst ? 'replace' : 'append'}&idInformacionF=${encodeURIComponent(idInformacionF)}`;
        const response = await fetch(url, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
      ...
      if (SERVER_MAX_PACKET_BYTES === 536870912 && productosSerializados.length > 10000) {
        console.info("[EmpaqueSecundario] Rendimiento óptimo: Con max_allowed_packet=512MB puede procesar lotes grandes eficientemente.");
      }
    } catch (error) {
